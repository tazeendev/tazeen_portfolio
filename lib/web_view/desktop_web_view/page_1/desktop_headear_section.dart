...
class _HeaderScreenState extends State<HeaderScreen> {
        ...
        children: [
          Row(
            children: [
              _buildMenu(leftMenu),
              const SizedBox(width: 20),
              _buildDecorativeLineWithCircle(length: 100, isCircleAtEnd: true),
            ],
          ),
          Image.asset('assets/logu/logo.png', height: 55),
          Row(
            children: [
              _buildDecorativeLineWithCircle(length: 100, isCircleAtEnd: false),
              const SizedBox(width: 20),
              _buildMenu(rightMenu),
              const SizedBox(width: 16),
              _buildGetStartedButton(),
  ...
  Widget _buildHoverMenu(String text) {
    bool isHovered = hoverMenu == text;
    bool isActive = widget.activeSection == text; // ✅ Active state check
    return MouseRegion(
      onEnter: (_) => setState(() => hoverMenu = text),
      onExit: (_) => setState(() => hoverMenu = ""),
      child: GestureDetector(
        onTap: () => widget.onNavTap(text), // ✅ Navigation callback
        child: AnimatedContainer(
          duration: const Duration(milliseconds: 300),
          margin: const EdgeInsets.symmetric(horizontal: 14),
          transform: Matrix4.identity()..scale(isHovered ? 1.2 : 1.0),
            ...
            children: [
              AnimatedDefaultTextStyle(
                ...
                style: TextStyle(
                  color: isActive
                      ? AppColors.primary
                      : (isHovered ? AppColors.primary : Colors.white70),
                  fontSize: 15,
                  letterSpacing: isHovered ? 1.2 : 0,
                  fontFamily: 'Inter',
                  fontWeight: FontWeight.w700,
  ...
  Widget _buildDecorativeLineWithCircle({required double length, required bool isCircleAtEnd}) {
    return Row(
      children: isCircleAtEnd
          ? [Container(width: length, height: 2, color: AppColors.primary), _buildCircle()]
          : [_buildCircle(), Container(width: length, height: 2, color: AppColors.primary)],
    );
  }
  ...
  Widget _buildCircle() {
    return Container(
      ...
      decoration: BoxDecoration(
        color: AppColors.primary,
        shape: BoxShape.circle,
        boxShadow: [BoxShadow(color: AppColors.primary.withOpacity(0.7), blurRadius: 12, spreadRadius: 3)],
      ),
    );
  }
  ...
  Widget _buildGetStartedButton() {
    return MouseRegion(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          boxShadow: isButtonHovered
              ? [BoxShadow(color: AppColors.primary.withOpacity(0.7), blurRadius: 15, spreadRadius: 4)]
              : [],
        ),
        child: ElevatedButton(
          onPressed: () {},
          style: ElevatedButton.styleFrom(
            backgroundColor: isButtonHovered ? AppColors.secondary : AppColors.primary,
